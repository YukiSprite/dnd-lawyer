...
# ============================================
# 综合相似度阈值（0-1）
# 综合相似度 = DEDUP_CONTENT_WEIGHT * 内容相似度 + DEDUP_PATH_WEIGHT * 路径相似度
# 如果综合相似度 > 此阈值，则认为两个文档是重复的
# 推荐值：0.50-0.70（考虑路径后，阈值可以降低）
DOCUMENT_SIMILARITY_THRESHOLD = 0.60

# 路径相似度权重（0-1）
# 越高越重视来源路径的差异，来自同一路径的文档更容易被判定为重复
# 推荐值：0.5-0.7（优先排除同来源的重复内容）
DEDUP_PATH_WEIGHT = 0.6

# 内容相似度权重（0-1）
# 越高越重视内容的相似性
# 注意：DEDUP_CONTENT_WEIGHT + DEDUP_PATH_WEIGHT 应该等于 1.0
DEDUP_CONTENT_WEIGHT = 0.4
...
DND_PATH_BOOST_RULES = {
    ...
    "2024": 0.1,             # 2024 版本规则 +0.15（最高优先级）
    "新贤者谏言": 0.1,
    "术语汇编":0.1,
    
    # 降权规则（负向加权）
    # 注意：降权值不要太大，否则可能完全过滤掉这些文档
    "2014": -0.15,       # 2014 版玩家手册 -0.15（与 2024 版抵消）
}
